/**
 * BunnySlider — reusable draggable slider with the stretching bunny thumb.
 * value: 0–100 (percentage along the track).
 */
...
export interface BunnySliderProps {
  ...
  marks?: number[]                       // default [0,25,50,75,100]
}

const DEFAULT_MARKS = [0, 25, 50, 75, 100]

function BunnyBack() {
  return (
    <svg width="17.3" height="17.3" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
      <g clipPath="url(#bs-bb-clip)">
...
function BunnyFront() {
  return (
    <svg width="32" height="48" viewBox="0 0 32 50" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <defs>
        <filter id="bs-fb-shadow" x="1.5" y="0" width="28" height="28" filterUnits="userSpaceOnUse" colorInterpolationFilters="sRGB">
        ...
        <linearGradient id="bs-fb-grad" x1="15.5" y1="1.5" x2="15.5" y2="25.5" gradientUnits="userSpaceOnUse">
...
export function BunnySlider({ value, onChange, marks = DEFAULT_MARKS }: BunnySliderProps) {
  ...
  const pctFromClientX = useCallback((clientX: number) => {
    ...
    return Math.max(0, Math.min(100, Math.round(((clientX - left - 10) / (width - 20)) * 100)))
  }, [])

  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    e.preventDefault()
    onChange(pctFromClientX(e.clientX))
    const onMove = (ev: MouseEvent) => onChange(pctFromClientX(ev.clientX))
    const onUp   = () => { window.removeEventListener('mousemove', onMove); window.removeEventListener('mouseup', onUp) }
    window.addEventListener('mousemove', onMove)
    window.addEventListener('mouseup',   onUp)
  }, [pctFromClientX, onChange])

  const handleTouchStart = useCallback((e: React.TouchEvent<HTMLDivElement>) => {
    onChange(pctFromClientX(e.touches[0].clientX))
    const onMove = (ev: TouchEvent) => onChange(pctFromClientX(ev.touches[0].clientX))
  ...
  return (
    ...
    >
      ...
      {/* Back body — always at 0% */}
      <div className="bs-back" aria-hidden="true"><BunnyBack /></div>
      ...
      {marks.map((m) => (
        <button
          key={m}
          type="button"
          className={`bs-stop${fillPct >= m ? ' bs-stop--filled' : ''}`}
