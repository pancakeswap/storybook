...
import { RouteDiagramV2, TokenChip } from "./RouteDiagramV2";
...
interface RouterModalV2Props {
  route: RouteV2;
  onClose?: () => void;
}

export function RouterModalV2({ route, onClose }: RouterModalV2Props) {
  const totalHops = route.branches.reduce((a, b) => a + b.hops.length, 0);
      ...
      <FooterArea>
        <StatLine>
          ...
          <Text fontSize="13px" bold>
            {route.branches.length} routes · {totalHops} hops
          </Text>
        </StatLine>
        <HintBox>
          The PancakeSwap Smart Router splits your trade across the best pools to minimise price
          impact and gas. Hover any pool to see its liquidity sources.
        </HintBox>
      </FooterArea>
    </ModalCard>
  );
