  ...
  return (
    <div className="perps-root" style={{ padding: 0 }}>
      <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        ...
        {/* Account summary */}
        <div style={{ background: 'var(--pcs-colors-input)', borderRadius: 'var(--p-radius-md)', overflow: 'hidden' }}>
          {([
            ['Portfolio Value',  `$${totalBalance}`,     'var(--pcs-colors-text)'  ],
            ['Available Margin', `$${availableBalance}`, 'var(--pcs-colors-success)'  ],
            ['In Positions',     `$${usedBalance}`,      'var(--pcs-colors-text)'  ],
          ] as [string, string, string][]).map(([label, value, color], i, arr) => (
            <div key={label} style={{
              display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              padding: '8px 12px',
              borderBottom: i < arr.length - 1 ? '1px solid var(--pcs-colors-card-border)' : 'none',
            }}>
        ...
        <div>
          ...
          <div className="p-input-wrap">
            <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--pcs-colors-text-subtle)', flexShrink: 0 }}>USDC</span>
        ...
        {rawAmount > rawMax && rawAmount > 0 && (
          <p style={{ textAlign: 'center', fontSize: 11, color: 'var(--pcs-colors-failure)', margin: 0 }}>
